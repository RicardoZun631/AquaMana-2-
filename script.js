...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function applySettings() {
    ...
    setBusiness.value         = settings.business;
    toggleLowStock.checked    = settings.lowStockAlert;
    toggleExpWarn.checked     = settings.expenseWarn;
    toggleDailyEmail.checked  = settings.dailyEmail;
    setThreshold.value        = settings.lowStockThreshold || 5;
  ...
  loginBtn.addEventListener('click', function() {
    ...
    if (!valid) return;
    showLoading(); loginBtn.disabled = true;
    auth.signInWithEmailAndPassword(email, pass)
      .catch(function(err) {
        ...
        signinError.textContent = msg; signinError.classList.remove('hidden');
      });
  });
  ...
  auth.onAuthStateChanged(function(user) {
    if (user) {
      ...
      db.collection('users').doc(user.uid).get()
        ...
        .catch(function() {
          settings.email = user.email;
          transactions = loadTx(); inventory = loadInv();
          showApp(user);
        });
    } else {
      currentUser = null; transactions = []; inventory = [];
      hideLoading();
...
//DashBoard
  function updateDashboard() {
    ...
    if (!transactions.length) {
      ...
      transactions.slice().sort(function(a,b){ return new Date(b.date)-new Date(a.date); })
  ...
  function buildMonthDropdown() {
    ...
    var months = {};
    transactions.forEach(function(tx) {
      var d=new Date(tx.date), key=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0');
      months[key]=true;
    });
    ...
    sorted.forEach(function(key){
      ...
      var opt=document.createElement('option'); opt.value=key; opt.textContent=label;
      monthSelect.appendChild(opt);
    });
    if (current && months[current]) { monthSelect.value=current; renderMonthlySummary(current); }
    else { monthSelect.value=''; monthlyCards.classList.add('hidden'); monthlyTxSec.classList.add('hidden'); monthlyHolder.classList.remove('hidden'); }
  }
  ...
  function renderMonthlySummary(key) {
    ...
    var p  = key.split('-'), yr = parseInt(p[0]), mo = parseInt(p[1]) - 1;
    var allMonthTxs = transactions.filter(function(tx) {
      var d = new Date(tx.date);
      return d.getFullYear() === yr && d.getMonth() === mo;
    });

    // Calculate totals from ALL transactions in month
    var inc = 0, exp = 0;
    ...
    if (monthlySearchQuery.trim()) {
      ...
      filtered = filtered.filter(function(tx){ return tx.title.toLowerCase().indexOf(q) !== -1; });
    }
  ...
  monthlyPrevBtn.addEventListener('click', function() { monthlyCurrentPage--; renderMonthlyPage(); });
  monthlyNextBtn.addEventListener('click', function() { monthlyCurrentPage++; renderMonthlyPage(); });
...
//Transactions
  function renderTable(){
    ...
    if(searchQuery.trim()){var q=searchQuery.toLowerCase();list=list.filter(function(t){return t.title.toLowerCase().indexOf(q)!==-1;});}
    ...
    list.forEach(function(tx){
      ...
      tr.innerHTML='<td>'+esc(tx.title)+'</td><td class="'+(tx.type==='income'?'amount-income':'amount-expense')+'">'+(tx.type==='income'?'+':'-')+fmt(tx.amount)+'</td>' +
        '<td><span class="badge badge-'+tx.type+'">'+tx.type+'</span></td><td class="tx-note-cell">'+esc(tx.note||'—')+'</td><td>'+fmtDate(tx.date)+'</td>' +
        '<td><div class="action-btns"><button class="btn-edit" onclick="AM.editTx(\''+tx.id+'\')">Edit</button><button class="btn-delete" onclick="AM.deleteTx(\''+tx.id+'\')">Delete</button></div></td>';
  ...
  productLinkToggle.addEventListener('click',function(){
    ...
    productLinkBody.classList.toggle('hidden',!h); productLinkArrow.classList.toggle('open',h);
  });
  ...
  function updateProdLinkInfo(){
    var pid=txProductSelect.value, qty=parseInt(txProdQty.value)||0;
    if(!pid){prodLinkInfo.classList.add('empty');prodLinkInfo.innerHTML='';return;}
    ...
    var warn=p.qty===0?'<div class="prod-link-info-stock-warn">⚠ Out of stock!</div>':p.qty<=LOW_STOCK?'<div class="prod-link-info-stock-warn">⚠ Low stock: '+p.qty+' left</div>':'';
    prodLinkInfo.innerHTML='<div class="prod-link-info-name">'+esc(p.name)+'</div><div class="prod-link-info-row">Price: '+fmt(p.price)+'/unit</div><div class="prod-link-info-row">In stock: '+p.qty+'</div>'+(qty>0?'<div class="prod-link-info-row">Total: <strong>'+fmt(qty*p.price)+'</strong></div>':'')+warn;
...
//Inventory
  ...
  imgUploadArea.addEventListener('dragleave',function(){imgUploadArea.classList.remove('drag-over');});
  imgUploadArea.addEventListener('drop',function(e){e.preventDefault();imgUploadArea.classList.remove('drag-over');var f=e.dataTransfer.files[0];if(f&&f.type.startsWith('image/'))readImageFile(f);});

  function readImageFile(file){
    if(file.size>2*1024*1024){toast('Image Too Large','Please use an image under 2MB.','error');return;}
    var r=new FileReader();r.onload=function(e){currentImgB64=e.target.result;showImgPreview(currentImgB64);};r.readAsDataURL(file);
  }
  function showImgPreview(src){imgPreview.src=src;imgPreview.classList.remove('hidden');imgPlaceholder.classList.add('hidden');removeImgBtn.classList.remove('hidden');}
  function clearImgPreview(){currentImgB64=null;imgPreview.src='';imgPreview.classList.add('hidden');imgPlaceholder.classList.remove('hidden');removeImgBtn.classList.add('hidden');imgFileInput.value='';}
  ...
  function renderInventory(){
    ...
    inventory.forEach(function(p){
      var isLow=p.qty<=LOW_STOCK, card=document.createElement('div');
      card.className='product-card'+(isLow?' low-stock':'');
      var imgHtml=p.image?'<img class="product-img" src="'+p.image+'" alt="'+esc(p.name)+'" />':'<div class="product-img-placeholder">📦</div>';
      card.innerHTML=imgHtml+'<div class="product-body"><div class="product-name">'+esc(p.name)+'</div><div class="product-desc">'+esc(p.desc||'')+'</div>' +
  ...
  invSaveBtn.addEventListener('click',function(){
    ...
    var qty=parseInt(invQty.value),price=parseFloat(invPrice.value);
    ...
    if(id){
      ...
      if(i!==-1){inventory[i].name=invName.value.trim();inventory[i].desc=invDesc.value.trim();inventory[i].qty=qty;inventory[i].price=price;if(currentImgB64!==null)inventory[i].image=currentImgB64;}
      exitInvEdit();
      toast('Product Updated', invName.value.trim() + ' has been updated.', 'success');
    } else {
      var newProd={id:genId(),name:invName.value.trim(),desc:invDesc.value.trim(),qty:qty,price:price,image:currentImgB64||null};
  ...
  function editProd(id){
    ...
    invEditId.value=p.id;invName.value=p.name;invDesc.value=p.desc||'';invQty.value=p.qty;invPrice.value=p.price;
        invFormTitle.textContent='Edit Product';invSaveBtn.textContent='Save Changes';invCancelBtn.classList.remove('hidden');
    if(p.image){showImgPreview(p.image);currentImgB64=null;}else{clearImgPreview();}
    document.querySelector('#section-inventory .form-card').scrollIntoView({behavior:'smooth',block:'start'});
  }

  function exitInvEdit(){
    invEditId.value='';invFormTitle.textContent='Add Product';invSaveBtn.textContent='Add Product';invCancelBtn.classList.add('hidden');
    invName.value='';invDesc.value='';invQty.value='';invPrice.value='';invNameError.textContent='';invQtyError.textContent='';invPriceError.textContent='';clearImgPreview();
  }
  ...
  function openSell(id){
    ...
    sellQtyInput.value='1';sellQtyError.textContent='';sellTotal.textContent=fmt(p.price);
    sellModal.classList.remove('hidden');sellQtyInput.focus();
  }
  sellQtyInput.addEventListener('input',function(){var p=inventory.find(function(p){return p.id===sellTargetId;});if(p)sellTotal.textContent=fmt((parseInt(this.value)||0)*p.price);});
  sellConfirmBtn.addEventListener('click',function(){
    sellQtyError.textContent='';
    var p=inventory.find(function(p){return p.id===sellTargetId;});if(!p)return;
    ...
    var total=qty*p.price; p.qty-=qty; saveInv();
    transactions.push({id:genId(),title:'Sale: '+p.name+' x'+qty,amount:total,type:'income',note:'Sold from Inventory',date:new Date().toISOString()});
    saveTx(); sellModal.classList.add('hidden'); sellTargetId=null;
    refreshAll();
...
//Settings
  saveProfileBtn.addEventListener('click',function(){
    ...
    settings.name=setName.value.trim(); settings.business=setBusiness.value.trim();
    db.collection('users').doc(currentUser.uid).update({name:settings.name,business:settings.business})
      .then(function(){
  ...
  saveCurrencyBtn.addEventListener('click',function(){
    ...
    if(currentUser) db.collection('users').doc(currentUser.uid).update({currency:settings.currency});
  });

  toggleLowStock.addEventListener('change',function(){
    settings.lowStockAlert=this.checked; renderInventory();
    toast(this.checked?'Low Stock Alerts ON':'Low Stock Alerts OFF', this.checked?'You will be notified when stock is low.':'Low stock alerts are now disabled.', this.checked?'success':'info');
    if(currentUser) db.collection('users').doc(currentUser.uid).update({lowStockAlert:settings.lowStockAlert});
  });

  toggleExpWarn.addEventListener('change',function(){
    settings.expenseWarn=this.checked; updateDashboard();
    toast(this.checked?'Expense Warning ON':'Expense Warning OFF', '', this.checked?'success':'info', 2000);
    if(currentUser) db.collection('users').doc(currentUser.uid).update({expenseWarn:settings.expenseWarn});
  });

  saveThresholdBtn.addEventListener('click', function() {
    var val = parseInt(setThreshold.value);
    if (isNaN(val) || val < 1) { toast('Invalid Threshold', 'Please enter a number greater than 0.', 'error'); return; }
    settings.lowStockThreshold = val;
    LOW_STOCK = val;
    renderInventory();
    toast('Threshold Updated', 'Low stock alerts will trigger at ' + val + ' or fewer units.', 'success');
    if (currentUser) db.collection('users').doc(currentUser.uid).update({ lowStockThreshold: val });
  });

  toggleDailyEmail.addEventListener('change',function(){
    settings.dailyEmail=this.checked;
    toast(this.checked?'Daily Emails ON':'Daily Emails OFF', this.checked?'You will receive a daily summary email.':'Daily summary emails are disabled.', this.checked?'success':'info');
...
testEmailBtn.addEventListener('click', function() {
  ...
  sendEmail(
    currentUser.email,
    'AguaMana — Test Email ✅',
    'Hi ' + (settings.name || 'there') + ',\n\n' +
    'This is a test email from AguaMana.\n\n' +
    ...
    setTimeout(function() {
      emailTestSent.classList.add('hidden');
    }, 4000);
    toast('Test Email Sent!', 'Check ' + currentUser.email, 'success', 4000);
  }).catch(function(err) {
    testEmailBtn.disabled = false;
    console.error('TEST EMAIL FULL ERROR:', err);
    console.error('TEST EMAIL JSON:', JSON.stringify(err, null, 2));
    toast('Email Failed ❌', (err && err.text) ? err.text : 'Check console for EmailJS details.', 'error', 6000);
  });
});
  ...
  resetDataBtn.addEventListener('click',function(){
    showConfirm(
      ...
      function() {
        ...
        localStorage.removeItem(txKey()); localStorage.removeItem(invKey());
        refreshAll();
  ...
  navItems.forEach(function(item){item.addEventListener('click',function(e){e.preventDefault();showSection(item.dataset.section);});});
